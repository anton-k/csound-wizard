...
public class MultitouchPositionX extends View implements OnKey2Listener {	
	private static int
	...
	public MultitouchPositionX(Context ctx, int maxTouch, int n, Range rangeY, List<String> labels, ColorParam colorParam, TextParam textParam) {
		super(ctx);
	...
	public MultitouchPositionX(Context ctx, AttributeSet attrs) {
		super(ctx, attrs);
	        ...
	        for (int size = event.getPointerCount(), i = 0; i < size; i++) {
	          ...
	          if (point != null) {        	
	            ...
	            if (mRect.contains(point)) {
	            	...
	            	mListener.move(pointerId, ViewUtils.getCell(mn, point.x),  mRangeY.fromRelative(point.y));	            			        	            	
	            } else {	            		
