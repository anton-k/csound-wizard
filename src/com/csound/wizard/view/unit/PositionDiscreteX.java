...
public class PositionDiscreteX extends View implements StatefulUnit, OnPressListener, OnSlideListener, OnTapListener {	
	private static int
	...
	private int mn = 12, mSelected = mn / 2;
	...
	private Range mRangeY;
	
	public PositionDiscreteX(Context ctx, String id, int selected, float y, int n, Range rangeY, List<String> labels, ColorParam colorParam, TextParam textParam) {
		super(ctx);
	...
	public PositionDiscreteX(Context ctx, AttributeSet attrs) {
		super(ctx, attrs);
	...
	public double[] getUnitState() {	
		return UnitUtils.getUnitStateIntegerFloatPair(mSelected, mRangeY.fromRelative(my));
	}
	...
	private void updateValue(float x, float y) {
		...
		if (Math.abs(mx - x1) + Math.abs(my - y1) > ViewUtils.EPS) {
			if (Math.abs(my - y1) > ViewUtils.EPS) {
				mSlideListener.slide(mRangeY.fromRelative(y1));
			}
			
			int selected = (int) (mn * x1);
