...
public class TrackState {
	...
	private void getStateFromJson(Object json) {
		...
		if (Json.getJson(Units.INIT, json) != null) {
			if (Json.getJson(Units.SLIDER, json) != null) {
				Float initVal = Json.getFloat(Units.INIT, json);
				if (initVal != null) {
					this.put(UnitUtils.getUnitId(Json.getJson(Units.SLIDER, json)), toTrackState(initVal));
				}
			} else if(Json.getJson(Units.TOGGLE, json) != null) {
				Boolean initVal = Json.getBoolean(Units.INIT, json);
				if (initVal != null) {
					this.put(UnitUtils.getUnitId(Json.getJson(Units.TOGGLE, json)), toTrackState(initVal));
				}				
			} else if (Json.getJson(Units.KNOB, json) != null) {
				Float initVal = Json.getFloat(Units.INIT, json);
				if (initVal != null) {
					this.put(UnitUtils.getUnitId(Json.getJson(Units.KNOB, json)), toTrackState(initVal));
				}
			} else if (Json.getJson(Units.SPINNER, json) != null) {
				Integer initVal = Json.getInteger(Units.INIT, json);
				if (initVal != null) {
					this.put(UnitUtils.getUnitId(Json.getJson(Units.SPINNER, json)), toTrackState(initVal));
				}	
			} else if (Json.getJson(Units.HOR_RADIO, json) != null) {
				Integer initVal = Json.getInteger(Units.INIT, json);
				if (initVal != null) {
					this.put(UnitUtils.getUnitId(Json.getJson(Units.HOR_RADIO, json)), toTrackState(initVal));
				}
			} else if (Json.getJson(Units.VER_RADIO, json) != null) {
				Integer initVal = Json.getInteger(Units.INIT, json);
				if (initVal != null) {
					this.put(UnitUtils.getUnitId(Json.getJson(Units.VER_RADIO, json)), toTrackState(initVal));
				}												
			} else if (Json.getJson(Units.CHESS, json) != null) {
				Entry<Integer,Integer> initVal = Json.getIntegerPair(Units.INIT, json);
				if (initVal != null) {
					this.put(UnitUtils.getUnitId(Json.getJson(Units.CHESS, json)), toTrackStateIntegerPair(initVal));
				}
			} else if (Json.getJson(Units.PLANE, json) != null) {
				Entry<Float,Float> initVal = Json.getFloatPair(Units.INIT, json);
				if (initVal != null) {
					this.put(UnitUtils.getUnitId(Json.getJson(Units.PLANE, json)), toTrackStateFloatPair(initVal));
				}
			} else if (Json.getJson(Units.PLANE_X, json) != null) {
				Entry<Integer,Float> initVal = Json.getIntegerFloatPair(Units.INIT, json);
				if (initVal != null) {
					this.put(UnitUtils.getUnitId(Json.getJson(Units.PLANE_X, json)), toTrackStateIntegerFloatPair(initVal));
				}
			} else if (Json.getJson(Units.PLANE_Y, json) != null) {
				Entry<Integer,Float> initVal = Json.getIntegerFloatPair(Units.INIT, json);
				if (initVal != null) {
					this.put(UnitUtils.getUnitId(Json.getJson(Units.PLANE_Y, json)), toTrackStateIntegerFloatPair(initVal));
				}
			} 			
		}		
