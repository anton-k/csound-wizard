...
public class MultitouchPosition extends View implements OnKey2Listener {	
	private static int
		desiredWidth = 500,
		desiredHeight = 500;
	...
	public MultitouchPosition(Context ctx, int maxTouch, Range rangeX, Range rangeY, ColorParam colorParam) {
		super(ctx);
	...
	public MultitouchPosition(Context ctx, AttributeSet attrs) {
		super(ctx, attrs);
	...
	private void on(int pointerId, PointF f) {
		mListener.on(pointerId, mRangeX.fromRelative(f.x), mRangeY.fromRelative(f.y));				
	}
	
	private void move(int pointerId, PointF f) {
		mListener.move(pointerId, mRangeX.fromRelative(f.x), mRangeY.fromRelative(f.y));				
	}
